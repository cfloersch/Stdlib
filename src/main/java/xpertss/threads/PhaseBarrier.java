 ...
 * generation of a phase barrier has an associated phase number. The
 * phase number starts at zero, and advances when all parties arrive at
 * the phase barrier, wrapping around to zero after reaching {@code
 * Integer.MAX_VALUE}. The use of phase numbers enables independent
 * control of actions upon arrival at a phase barrier and upon awaiting
 ...
 *       number</em>; that is, the phase number of the phase barrier
 *       to which the arrival applied. When the final party for a
 *       given phase arrives, an optional action is performed and the
 *       phase advances.  These actions are performed by the party
 ...
 *       the phase barrier advances to (or is already beyond) the phase.
 *       Unlike similar constructions using {@code CyclicBarrier},
 *       method {@code awaitAdvance} continues to wait even if the
 *       waiting thread is interrupted. Interruptible and timeout
 *       versions are also available, but exceptions encountered while
 *       tasks wait interruptibly or with timeout do not change the
 *       state of the phase barrier. If necessary, you can perform any
 *       associated recovery within handlers of those exceptions,
 *       often after invoking {@code forceTermination}.  Phase barriers
 *       may also be used by tasks executing in a {@link ForkJoinPool},
 ...
 * parties to become zero.  As illustrated below, when phase barriers
 * control actions with a fixed number of iterations, it is often
 * convenient to override this method to cause termination when the
 ...
 * non-zero (as established in the {@link #PhaseBarrier(PhaseBarrier,int)}
 * constructor, {@link #register}, or {@link #bulkRegister}), the
 * child phase barrier is registered with its parent. Whenever the number
 ...
 * }}</pre>
 *
 * If the main task must later await termination, it may re-register and
 * then execute a similar loop:
 *  <pre> {@code
 *   // ...
 *   phaser.register();
 *   while (!phaser.isTerminated())
 ...
 *   phaser.register(); // assumes caller not already registered
 ...
 * upon construction. After invocation of {@code build(new Task[n], 0, n,
 * new PhaseBarrier())}, these tasks could then be started, for example by
 * submitting to a pool:
 *
 *  <pre> {@code
 * void build(Task[] tasks, int lo, int hi, PhaseBarrier ph) {
 *   if (hi - lo > TASKS_PER_BARRIER) {
 *     for (int i = lo; i < hi; i += TASKS_PER_BARRIER) {
 *       int j = Math.min(i + TASKS_PER_BARRIER, hi);
 *       build(tasks, i, j, new PhaseBarrier(ph));
 *     }
 *   } else {
 *     for (int i = lo; i < hi; ++i)
 *       tasks[i] = new Task(ph);
 ...
 * }}</pre>
 *
 * The best value of {@code TASKS_PER_BARRIER} depends mainly on
 ...
 * phase barriers to accommodate arbitrarily large sets of participants.
 */
public class PhaseBarrier {
   ...
   private static boolean shouldWait(int currentPhase, int phase)
   {
      return currentPhase >= 0 && currentPhase <= phase;
   }

   /**
    * The parent of this phaser, or null if none
    */
   private final PhaseBarrier parent;

   /**
    * The root of phaser tree. Equals this if not in a tree.
    */
   private final PhaseBarrier root;
   ...
   private int doArrive(boolean deregister) {
      ...
      final PhaseBarrier root = this.root;
      for (;;) {
   ...
   private int doRegister(int registrations) {
      ...
      final PhaseBarrier parent = this.parent;
      int phase;
      for (;;) {
         long s = state.get();
         int counts = (int)s;
         int parties = partiesOf(counts);
         ...
         if (registrations > MAX_PARTIES - parties) {
            ...
            synchronized (this) {               // 1st sub registration
               long cState = state.get();
               if (cState == s) {               // recheck under lock
                  parent.doRegister(1);
                  do {                        // force current phase
                     phase = phaseOf(root.state.get());
                     // assert phase < 0 || (int)state == EMPTY;
                  } while (!state.compareAndSet(cState, ((long) phase << PHASE_SHIFT) | adj));
                  break;
               }
            }
         }
   ...
   private long reconcileState() {
      ...
      if (root != this) {
         ...
         while ((phase =phaseOf(root.state.get())) != phaseOf(s)
               && !state.compareAndSet(s, s = (((long)phase << PHASE_SHIFT) |
               (s & PARTIES_MASK) |
               ((p = partiesOf(s)) == 0 ? EMPTY :
   ...
   /**
    * Equivalent to {@link #PhaseBarrier(PhaseBarrier, int) PhaseBarrier(parent, 0)}.
    *
    * @param parent the parent phaser
    */
   public PhaseBarrier(PhaseBarrier parent) {
      this(parent, 0);
   }
    ...
    * or greater than the maximum number of parties supported
    */
   public PhaseBarrier(PhaseBarrier parent, int parties) {
      if (parties >>> PARTIES_SHIFT != 0)
         throw new IllegalArgumentException("Illegal number of parties");
      int phase = 0;
      this.parent = parent;
      if (parent != null) {
         final PhaseBarrier root = parent.root;
         this.root = root;
    ...
    *
    * TODO Rename to register(int parties)..
    * The bulk nature should be obvious
    * TODO Should I waste my time returning the phase
    *
   ...
   /**
    * Arrives at this phase barrier and awaits others. Equivalent in effect
    * to {@code awaitAdvance(arrive())}.  If you need to await with interruption
      ...
      for (;;) {
         ...
         if (phase < 0) {
            ...
            if (!state.compareAndSet(s, n))
               return phaseOf(state.get()); // terminated
            releaseWaiters(phase);
            return nextPhase;
         }
      }
   ...
   // TODO Do I need a TimeoutException here or just change the return to a boolean
   // They use a negative value to indicate terminated. Maybe I could specify a negative value for timeout?
   public int arriveAndAwaitAdvance(long duration, TimeUnit unit)
   {
      return 0;
   }
    ...
    * the phase barrier.
    *
    * TODO Do we gain anything by returning an int????
    *
    * @param phase The phase number to await completion of
    * @return the current phase number if greater than the specified phase
    *    number, a negative number if the phase barrier is terminated, or
    *    a phase number one greater than the argument once awaiting has
    *    completed.
    */
    ...
    *
    * TODO Get rid of interruptible.. Keep timeout
    * awaitAdvance(int phase, long timeout, TimeUnit unit)
    * Do I want to throw TimeoutException??
    * Maybe return boolean instead of int and use that to determine timeout or not
    *
    * @param phase an arrival phase number, or negative value if
    * terminated; this argument is normally the value returned by a
    * previous call to {@code arrive} or {@code arriveAndDeregister}.
   ...
   // TODO Returns true if terminated, false if timed out
    ...
    *
    * <pre> {@code
    * PhaseBarrier phaser = new PhaseBarrier() {
   ...
   private void releaseWaiters(int phase) {
      ...
      while ((q = head.get()) != null && q.phase != phaseOf(root.state.get())) {
   ...
   private int internalAwaitAdvance(int phase, QNode node) {
      ...
      long s;
      int p;
      while (shouldWait(p = phaseOf(s = state.get()), phase)) {
         if (node == null) {           // spinning in noninterruptible mode
            ...
            if ((q == null || q.phase == phase) && phaseOf(state.get()) == phase) // avoid stale enq
      ...
      if (node != null) {
         ...
         if (p == phase && (p = phaseOf(state.get())) == phase)
   ...
   static final class QNode implements ForkJoinPool.ManagedBlocker {
      ...
      QNode(PhaseBarrier phaser, int phase, boolean interruptible, boolean timed, long nanos) {
         this.phaser = phaser;
         this.phase = phase;
      ...
      public boolean isReleasable() {
         if (thread == null)
            return true;
         if (!shouldWait(phaser.getPhase(), phase)) {
            thread = null;
            return true;
         }
