 ...
 * point should be treated. The tables are broadly classified into
 * <ul>
 ...
 * standard. Those forms have since been fixed and this implementation utilizes
 * a normalizer that operates on the more recent and correct forms. As such it
 * is incompatible for a very small (and highly unlikely) set of characters
 * with implementations which follow the spec to the letter. More information
 * on this issue in the normalization forms can be found
 * <a href="http://www.unicode.org/review/pr-29.html">here</a>
 */
@SuppressWarnings("UnusedDeclaration")
public abstract class StringPrep {

   private static final SaslPrep SASL_PREP = new SaslPrep();
   private static final NamePrep NAME_PREP = new NamePrep();
   private static final NodePrep NODE_PREP = new NodePrep();
   private static final ResourcePrep RESOURCE_PREP = new ResourcePrep();
   private static final ISCSIPrep ISCSI_PREP = new ISCSIPrep();
   private static final TracePrep TRACE_PREP = new TracePrep();
    ...
    * @return A StringPrep instance of the specified profile
    * @throws NullPointerException if the specified profile is {@code null}
    */
   public static StringPrep getInstance(Profile profile)
   {
      switch(profile) {
         case SASLPrep:
            return SASL_PREP;
   ...
   private boolean doBidiCheck;

   StringPrep(boolean doNormalize, boolean doBidiCheck)
   {
      this.doNormalize = doNormalize;
      this.doBidiCheck = doBidiCheck;
   }
    ...
    *    will remain compatible over long periods of time. Stored strings MUST
    *    NOT contain unassigned characters.</li>
    *    <li><b>Query</b> strings which typically have a short lifespan. These
    *    strings MAY contain unassigned characters.</li>
    * </ol>
    *
    * @param src The string to prepare for encoding
    * @param allowUnassigned {@code true} to allow unassigned characters
    * @return The prepared string
    * @throws SyntaxException If an a prohibited character was encountered or
    *    the string failed the bidirectional text checks
    */
   public String prepare(String src, boolean allowUnassigned) throws SyntaxException
   {
      // Mapping (iteration #1)  - 3 Iterations kind of sucks
      StringBuilder buf = new StringBuilder();
      for(int i = 0; i < src.length(); i++) {
         int srcCodePoint = src.codePointAt(i);
         if(!allowUnassigned && isUnassigned(srcCodePoint))
            throw new SyntaxException(
               format("unassigned code point %s found in input at position %d ",
                  Integer.toHexString(srcCodePoint), i));
         int[] codePoints = mapCodePoint(srcCodePoint);
         if(codePoints == null) {
            buf.appendCodePoint(srcCodePoint);
         } else {
            for(int codePoint : codePoints) buf.appendCodePoint(codePoint);
         }
      }

      // Normalization (Iteration #2)
      if(doNormalize) {
         src = Normalizer.normalize(
                        buf, Normalizer.Form.NFKC);
      }


      boolean hasRorAL = false, hasL = false;
      BidiCat first = null, last = null;

      // Check for prohibited chars (Iteration #3)
      int len = src.length();
      for(int i = 0; i < len; i++) {
         int codePoint = src.codePointAt(i);
         if(prohibit(codePoint)) {
            throw new SyntaxException(
               format("prohibited code point %s found in output at position %d ",
                  Integer.toHexString(codePoint), i));
         }

         // Do this here to avoid a fourth iteration
         if(doBidiCheck) {
            BidiCat bCat = checkBidiCat(codePoint);
            if (bCat == BidiCat.RorAL) {
               hasRorAL = true;
            } else if (bCat == BidiCat.L) {
               hasL = true;
            }
            if(i == 0) {
               first = bCat;
      ...
      if(doBidiCheck) {
         if(hasRorAL) {
            if(hasL) throw new SyntaxException("mixed bidi detected in output");
            if(first != BidiCat.RorAL)
               throw new SyntaxException("bidi mismatch on first char");
            if(last != BidiCat.RorAL)
               throw new SyntaxException("bidi mismatch on last char");
         }
      }
   ...
   abstract int[] mapCodePoint(int codePoint);

   abstract boolean prohibit(int codePoint);


   private boolean isUnassigned(int codePoint)
   {
      return contains(unassigned, codePoint);
   }

   private BidiCat checkBidiCat(int codePoint)
   {
      if(contains(bdRorAL, codePoint)) return BidiCat.RorAL;
      if(contains(bdL, codePoint)) return BidiCat.L;
      return null;
   }
   ...
   int[] map(int[][] table, int codePoint)
   {
      int low = 0;
      ...
      while (low <= high) {
         int mid = (low + high) >>> 1;
         int[] midVal = table[mid];
         if (midVal[0] < codePoint) {
            low = mid + 1;
         } else if (midVal[0] > codePoint) {
            high = mid - 1;
         } else {
            return Arrays.copyOfRange(midVal, 1, midVal.length);
         }
      }
      return null;
   }

   boolean contains(int[][] table, int codePoint)
   {
      int low = 0;
      int high = table.length - 1;
   ...
   // bi, 0xdirectional markers R or AL
   private static final int[][] bdRorAL = {
      { 0x05BE, 0x05BE },
      { 0x05C0, 0x05C0 },
      { 0x05C3, 0x05C3 },
      { 0x05D0, 0x05EA },
